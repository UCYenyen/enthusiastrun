...
export async function POST(request: Request) {
  try {
    ...
    if (!registration) {
      ...
      return NextResponse.json(
        { message: `❌ Error: Registration ID '${registrationId}' not found.`, userData: null },
        { status: 404 }
      );
    }
    ...
    } else if (registration.qrCodeClaimed === true) {
        // 409: Sudah pernah check-in
        return NextResponse.json(
            { 
                message: `❌ Already Checked In: User ${registration.fullName} checked in on ${new Date(registration.qrCodeClaimedAt!).toLocaleString()}.`,
                userData: registration,
            },
            { status: 409 }
        );
