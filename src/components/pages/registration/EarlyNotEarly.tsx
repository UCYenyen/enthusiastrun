...
export default function EarlyNotEarly(props: EarlyNotEarlyProps) {
  return (
    <Link href={`${props.isActive ? props.href : '#'}`} className="relative w-full flex justify-center items-center">
      {props.isActive && !props.isSoldOut ? null : <div className='absolute w-full bg-black/25 h-full backdrop-blur-xs opacity-50'></div>}
      ...
      <h2 className="text-4xl font-impact text-center text-white mb-2">{!props.isSoldOut ? props.title : <span className='text-red-800 font-regular font-impact'>SOLD OUT</span>}</h2>
      {/* {props.price && (
