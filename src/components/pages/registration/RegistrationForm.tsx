...
export default function RegistrationForm({ category, type, mahasiswaUCEnabled = false }: RegistrationFormProps) {
  ...
  useEffect(() => {
    if (packageType === "bundling") {
      ...
      setParticipants(Array(participantCount).fill(null).map((_, i) => participants[i] || { ...emptyParticipant }));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (totalPrice > 0 && (!paymentProofUrl || !rekeningName)) return toast.error("Upload payment proof");
    ...
    <form onSubmit={handleSubmit} className="w-full max-w-4xl mx-auto">
      <div className="bg-white p-6 shadow-lg">
        ...
        <div className={`grid gap-4 ${mahasiswaUCEnabled && type === "early_bird" ? "grid-cols-1 md:grid-cols-3" : "grid-cols-1 md:grid-cols-2"}`}>
          ...
          <button type="button" onClick={() => setPackageType("bundling")} className={`p-6 rounded-xl border-4 ${packageType === "bundling" ? "border-[#4BCFFC] bg-[#4BCFFC]/10" : "border-gray-200"}`}>
      ...
      {totalPrice > 0 && (
        <div className="bg-white p-6 shadow-lg border-b-2 border-gray-100">
          ...
          <input type="text" required value={rekeningName} onChange={(e) => setRekeningName(e.target.value)} placeholder="Account holder name" className="w-full px-4 py-3 border-2 rounded-lg border-gray-200 text-background outline-none" />
        </div>
      )}
      ...
      {totalPrice > 0 && (
        <div className="bg-white p-6 shadow-lg">
          ...
          {!paymentProofUrl ? (
            <UploadWidget folder="enthusiast-run/payment-proofs" allowedFormats={["jpg", "png", "pdf"]} onUploadSuccess={(url, id) => { setPaymentProofUrl(url); setPaymentProofId(id || ""); }} label="" />
          ) : (
            <div className="p-4 bg-green-50 text-green-700">Proof uploaded. <button type="button" onClick={() => setPaymentProofUrl("")} className="underline">Change</button></div>
          )}
        </div>
      )}
